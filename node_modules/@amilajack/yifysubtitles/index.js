...
const runConditional = (imdbId, opts, res) => {
  return Promise.resolve(langFilter(res, opts.langs.map(formatLangLong))).then(
    res => downloads(res, opts)
  );
};
...
const yifysubtitles = (imdbId, opts) => {
  ...
  return scrape(imdbId).then(res =>
    res.length ? runConditional(imdbId, opts, res) : []
  );
};
